...
- **OJO — toda edición de skill vive en DOS lugares, siempre.** `~/.claude/skills/` es la **copia instalada** (no tiene `.git`); la fuente única de verdad es el repo `kumo-skills` — búscalo en el disco: que la carpeta instalada no sea un repo git NO significa que no exista el repo. Cualquier skill que edites (`desarrollo-riguroso`, las `doc-*`, esta misma) se commitea en el repo **y** se copia a la carpeta instalada en el mismo acto. Editar solo la copia instalada crea **drift bidireccional silencioso**: nadie lo ve hasta que las dos versiones divergen en direcciones opuestas. *(Caso real: la copia instalada de `desarrollo-riguroso` acumuló cinco lecciones de retros que nunca llegaron al repo, mientras el repo recibió una mejora que nunca llegó a la copia — lo descubrió el usuario preguntando, no el sistema.)*
