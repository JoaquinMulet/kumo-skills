...
8. **README actualizado Y COMPRENDIDO** — el `README.md` es la puerta de entrada de un integrante nuevo: si una skill no aparece ahí, para él no existe. Toda skill nueva entra en la tabla, en el mapa del conjunto y —si es un grafo de agentes— en la tabla de qué pone cada skill en cada paso. El gate verifica la EXISTENCIA de cada nodo y el numeral escrito a mano. **Pero un diagrama es un documento y se testea como documento:** si tocaste los diagramas o el mapa, pásalos por una **prueba de uso** (`doc-prueba-de-uso`) — un lector frío débil, leyendo SOLO el `README.md`, debe (a) explicar el ciclo con sus palabras, (b) elegir la skill correcta para tres situaciones concretas, y (c) decir qué comparten las skills-grafo. Lo que no pueda, es defecto del diagrama. Dos reglas de fondo:

   - **La estructura se DIBUJA, no se describe.** Si la propiedad que importa es estructural —paralelismo, ramificación excluyente, ciclo, convergencia—, tiene que estar en los **nodos y las flechas**, no en el texto dentro de una caja. Una caja que dice «varios agentes atacan cada afirmación» es prosa metida en un diagrama: dibuja los agentes y sus flechas. Una bifurcación excluyente se dibuja con un **rombo**, no con dos flechas idénticas cuyas etiquetas dicen «si…».

     **Cómo se corre la prueba** (borrar el texto de las cajas y ver qué queda): la pregunta NO es «¿el dibujo desnudo cuenta toda la historia?» — eso es inalcanzable y calificar contra esa vara castiga a cualquier diagrama. La pregunta es acotada: **¿se distingue secuencial de paralelo, cadena de ciclo, y excluyente de simultáneo?** La semántica —qué es cada caja, cuál rama se toma— vive legítimamente en las etiquetas. Y usa símbolos convencionales: un círculo `(( ))` no significa nada en un flowchart y obliga a leer la etiqueta para entender la forma.

     *(Dos casos reales. Uno: el paralelismo —lo único que distingue estas skills de una lista de pasos— quedó como enumeración dentro de un rectángulo, y el usuario preguntó «¿esto no debería mostrarse como un grafo?». Dos: al correr la prueba de verdad sobre los otros diagramas, aparecieron dos bifurcaciones excluyentes dibujadas como flechas gemelas — un defecto que yo había declarado ausente releyéndolos desde la silla del autor, que es la forma más débil de verificación.)*
   - Los diagramas son una **segunda fuente de verdad** del conjunto — se quedan al nivel de la topología (que cambia poco) y nunca reproducen el detalle del `SKILL.md`, que es donde vive el flujo real.

   *(Dos casos reales, en el mismo mes: el README decía «las siete» con ocho skills en disco —una skill entera invisible para quien llegara por GitHub—, y la primera versión de los diagramas se declaró lista con el gate verde y el usuario respondió «se ven pero no los entiendo para nada». El gate medía que el nodo existiera, no que el dibujo enseñara. Errores de diseño encontrados después, con la prueba de uso: rotular las cajas con nombres de skills, que son jerga interna; mezclar un ciclo de vida y un catálogo de herramientas en un mismo dibujo, dejando una skill sin ninguna flecha; y explicar el andamiaje con cinco sustantivos abstractos en vez de un caso concreto.)*
